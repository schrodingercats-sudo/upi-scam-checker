#!/usr/bin/env python3
"""
🚨 Simple Massive Scam Dataset Generator
...
def generate_massive_dataset(target_size=1000000):
    ...
    base_scams = [
        ...
        "High returns guaranteed: Invest ₹{amount} and get ₹{return_amount} in 15 days.",
        "Quick loan approval: Get ₹{amount} loan. Pay ₹{fee} processing fee.",
        "Crypto investment: Invest ₹{amount} and get ₹{return_amount} in 24 hours.",
    ...
    for i in range(target_size // 2):
        ...
        try:
            ...
            scam_text = template.format(
                ...
                return_amount=str(int(float(np.random.choice(amounts).replace(',', '')) * 2)
            )
